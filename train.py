...
def train(model: Hidden,
    ...
    for epoch in range(train_options.start_epoch, train_options.number_of_epochs + 1):
        ...
        for image, _ in train_data:
            ...
            message2 = torch.Tensor(np.random.choice([0, 1], (image.shape[0], hidden_config.message_length))).to(device)

            # to print the loss information and the metrics per step/epoch
            losses, _ = model.train_on_batch([image, message, message2])
        ...
        for image, _ in val_data:
            ...
            losses, (encoded_images, noised_images, decoded_messages, decoded_message2) = \
                model.validate_on_batch([image, message, message2])
        ...
        logging.info('Epoch {} validation duration {:.2f} sec'.format(epoch, validation_duration))
