...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, config: HiDDenConfiguration, noiser: Noiser):
        ...
        self.decoder1 = Decoder(config)
        self.decoder2 = Decoder(config)

    def forward(self, image, message1, message2):
        encoded_image = self.encoder(image, message1, message2)
        noised_and_cover = self.noiser([encoded_image, image])
        noised_image = noised_and_cover[0]
        decoded_message1 = self.decoder1(noised_image)
        decoded_message2 = self.decoder2(noised_image)
        return encoded_image, noised_image, decoded_message1, decoded_message2
